...
<%
	...
	String memGrade = (String)request.getAttribute("memGrade");
%>
...
<table border="1">
		<tr>
			<td>게시글 번호 : <%=noticeVO.getNoticeNum() %></td>
				<td> </td>
		</tr>
		<tr>
			<td>게시글 제목 : <%=noticeVO.getNoticeTitle() %></td>
			<td> &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp작성자 : <%=noticeVO.getNoticeWriter() %> &nbsp&nbsp등록 날짜 : <%=noticeVO.getNoticeDate() %></td>
		</tr>
		...
		<tr>
			<td colspan="2">
				<a href="<%=request.getContextPath() %>/HW/notice/list.do">[목록]</a>
				<% 
				String sessionId = (String)session.getAttribute("signedUser");
				String memId = noticeVO.getMemId();
				if(memGrade.equals("G004")){
					%>                 
				<a href="<%=request.getContextPath() %>/HW/notice/update.do?noticeNum=<%=noticeVO.getNoticeNum()%>">[게시글 수정]</a>
				<a href="<%=request.getContextPath() %>/HW/notice/delete.do?noticeNum=<%=noticeVO.getNoticeNum()%>">[게시글 삭제]</a>
					<% 
				}else {

package kr.or.ddit.exerciseNotice.servlet;
...
@WebServlet("/ListExerciseNoticeServlet")
public class ListExerciseNoticeServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			if ("exerciseNoticeList".equals(flag)) { // 목록조회
				// 스트링을쓰고 equals를 쓰면 에러가 나지않는다
				// 왜냐하면 flag가 널일수도있기 때문에;
				List<ExerciseNoticeVO> exerciseNoticeList = retrieveExerciseList(req);

				// 브라우저로 전달한 결과를 request에 attribute로 세팅
				req.setAttribute("exerciseNoticeList", exerciseNoticeList);

				RequestDispatcher disp = req.getRequestDispatcher("/HW_video/common/exerciseNoticeListResult.jsp");
	...
	private List<ExerciseNoticeVO> retrieveExerciseList(HttpServletRequest req) throws SQLException {

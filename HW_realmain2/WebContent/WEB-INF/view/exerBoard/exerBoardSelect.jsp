...
<table border="1">
      <tr>
         <td>게시글 번호 : <%=exerBoardVO.getExerBoardNum() %></td>
            <td> </td>
      </tr>
      <tr>
         <td>게시글 제목 : <%=exerBoardVO.getExerBoardTitle() %></td>
         <td> &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp작성자 아이디 : <%=exerBoardVO.getMemId() %> &nbsp&nbsp등록 날짜 : <%=exerBoardVO.getExerBoardDate() %></td>
      </tr>
      
      <tr>
         <td>게시글 내용 : </td>
         <td><%=exerBoardVO.getExerBoardContent() %></td>
      </tr>
     ...
     <%--  <tr>
      ...
      <tr>
         <td colspan="2">
            <a href="<%=request.getContextPath() %>/HW/exerboard/list.do">[목록]</a>
            <% 
            String sessionId = (String)session.getAttribute("signedUser");
            String memId = exerBoardVO.getMemId();
            if(memId.equals(sessionId)){
               %>                 
            <a href="<%=request.getContextPath() %>/HW/exerboard/update.do?exerBoardNum=<%=exerBoardVO.getExerBoardNum()%>">[게시글 수정]</a>
            <a href="<%=request.getContextPath() %>/HW/exerboard/delete.do?exerBoardNum=<%=exerBoardVO.getExerBoardNum()%>">[게시글 삭제]</a>
               <% 
            }   
               %>
         </td>

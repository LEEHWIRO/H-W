...
<table border="1">
		<tr>
			<td>게시글 번호 : <%=qnaVO.getQnaNum() %></td>
				<td> </td>
		</tr>
		<tr>
			<td>게시글 제목 : <%=qnaVO.getQnaTitle() %></td>
			<td> &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp작성자 아이디 : <%=qnaVO.getMemId() %> &nbsp&nbsp등록 날짜 : <%=qnaVO.getQnaDate() %></td>
		</tr>
		
		<tr>
			<td>게시글 내용 : </td>
			<td><%=qnaVO.getQnaContent() %></td>
		</tr>
		
		<tr>
			<td colspan="2">
				<a href="<%=request.getContextPath() %>/HW/qna/list.do">[목록]</a>
				<% 
				String sessionId = (String)session.getAttribute("signedUser");
				String memId = qnaVO.getMemId();
				if(memId.equals(sessionId)){
					%>                 
				<a href="<%=request.getContextPath() %>/HW/qna/update.do?qnaNum=<%=qnaVO.getQnaNum()%>">[게시글 수정]</a>
				<a href="<%=request.getContextPath() %>/HW/qna/delete.do?qnaNum=<%=qnaVO.getQnaNum()%>">[게시글 삭제]</a>
					<% 
				}	
					%>
			</td>
